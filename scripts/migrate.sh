#!/bin/bash
# Apply a book_track migration via the shared infra migrate script.
#
# Usage (from Flutter/book_track):
...
exec "$INFRA_ROOT/migrate.sh" "${MODE_ARGS[@]}" --app book_track "$MIGRATION_FILE"
