...
"""
Book Track - iPhone Deployment Script
...
class IPhoneDeployment:
    ...
    def check_if_deployment_needed(self) -> None:
        ...
        if os.path.exists(self.hash_file):
            try:
                ...
                # Handle both nested {'hashes': {...}} and legacy flat {...} formats
                stored_hashes = stored_data.get('hashes') if 'hashes' in stored_data else stored_data
                last_deploy_time = stored_data.get('last_deploy_time') if isinstance(stored_data, dict) else None
    ...
    def check_if_rebuild_needed(self) -> bool:
        ...
        try:
            with open(self.hash_file, 'w') as f:
                json.dump({'hashes': current_hashes}, f, indent=2)
        except IOError:
    ...
    def check_prerequisites(self) -> None:
        ...
        if not self.check_project_directory():
            raise RuntimeError("pubspec.yaml not found. Please run this script from the project root directory.")
    ...
    def deploy(self) -> None:
        ...
        self.print_warning("VPN detection does not work. Disable any active VPN to prevent deployment from stalling indefinitely after the build completes.")
        self.print_info("Checking for iOS devices...")
        devices = self._list_ios_devices()

        if len(devices) != 1:
            if not devices:
                raise RuntimeError("No physical iPhone detected. Unlock and connect via USB or Wi-Fi and retry.")
